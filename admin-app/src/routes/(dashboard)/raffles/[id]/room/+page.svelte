<script lang="ts">
   ...
   *  one identity ("251 Lottery Team") regardless of which admin is signed
...
<svelte:head><title>Room · {raffle?.title ?? 'Raffle'} · 251 Lottery Admin</title></svelte:head>
  ...
  <div class="flex h-[60vh] flex-col rounded-card border border-border bg-card">
              ...
              {#if groupStart}
                <p class="mb-1 flex items-baseline gap-1.5 px-1 text-[11px]">
                  <span class="font-bold {isAdmin ? 'text-primary' : 'text-ink'}">
                    {isAdmin ? '251 Lottery Team' : (message.senderName ?? 'Ticket holder')}
                  </span>
    ...
    <form on:submit|preventDefault={send} class="flex items-center gap-2 border-t border-border p-3">
      <input
        ...
        bind:value={draft}
        maxlength="500"
        placeholder="Post as 251 Lottery — links allowed"
