<script lang="ts">
  ...
  let regeneratingTier: number | null = null;
  ...
  // Force this tier's draw process to (re)start right now, for a DIFFERENT
  // random participant than whoever currently holds it — whether that link
  // is still active, already expired, or hasn't been generated yet. This
  // is the manual version of what trigger-expiry-check.job.ts already does
  // automatically every ~60s to any link past its expiry.
  async function regenerateTrigger(tier: number) {
    if (!engine || regeneratingTier !== null) return;
    regeneratingTier = tier;
    try {
      const response = await api.post<{ trigger: { delivery: 'sent' | 'demo' | 'failed'; link: string } }>(
        `/admin/raffles/${raffleId}/draw-trigger/reassign`,
        { tier, reason: 'Regenerated by admin — new random participant selected' }
      );
      if (response.trigger.delivery === 'sent') {
        toast.success(`${ordinal(tier)} prize invitation regenerated for a new participant.`, 'Regenerated');
      } else {
        resentTriggerLinks = { ...resentTriggerLinks, [tier]: response.trigger.link };
        toast.info(`${ordinal(tier)} prize invitation regenerated — demo mode, copy the link below to test.`, 'Link Ready');
      }
      await load();
    } catch (cause) {
      toast.error(apiErrorMessage(cause, `Could not regenerate the ${ordinal(tier)} prize invitation.`), 'Regenerate Failed');
    } finally {
      regeneratingTier = null;
    }
  }
  ...
  {:else if engine}
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
      ...
      <!-- ── Right: Prize Draw Links (random trigger recipients) — fully
           ...
           auto-reassigns it if it expires unclicked (now a 5-minute
           window — see TRIGGER_TTL_MS). Two manual overrides: Resend (same
           participant, fresh link — resendTrigger above) and Regenerate
           (a different participant, right now, instead of waiting out the
           5 minutes — regenerateTrigger above). ── -->
      <div>
            ...
            {#each engine.prizes as prize (prize.tier)}
              ...
              <div class="rounded-button border border-border bg-card p-4 transition-all shadow-xs hover:border-primary/30 {!unlocked && !trigger ? 'opacity-60' : ''}">
                ...
                {#if unlocked && !drawn}
                  <button
                    type="button"
                    class="admin-press mt-3 flex h-7 items-center gap-1.5 rounded-button border border-border bg-bg px-2.5 text-[11px] font-bold text-muted hover:border-primary/40 hover:text-primary disabled:opacity-50"
                    title="Expire the current link (if any) and send to a different random participant now"
                    disabled={regeneratingTier !== null}
                    on:click={() => regenerateTrigger(prize.tier)}
                  >
                    {#if regeneratingTier === prize.tier}
                      <RefreshCw size={11} class="animate-spin" /> Regenerating…
                    {:else}
