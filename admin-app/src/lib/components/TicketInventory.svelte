...
<style>
  ...
  .inventory-legend { display: flex; gap: 18px; margin-top: 22px; font-size: 12px; } .held-label { color: #805e14; } .sold-label { color: #0135c6; } nav { display: flex; justify-content: space-between; align-items: center; font-size: 12px; font-variant-numeric: tabular-nums; }
  .inventory-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(68px, 1fr)); gap: 8px; } .inventory-grid > span { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 52px; border: 1px solid #d7dce6; border-radius: 8px; font-size: 13px; font-variant-numeric: tabular-nums; } .inventory-grid small { font-size: 9px; color: #5b6472; margin-top: 2px; } .inventory-grid .held { background: #fff4d6; border-color: #c8a456; } .inventory-grid .sold { background: #e1e9fc; border-color: #9fb8e8; }
