<script lang="ts">
  ...
  import { ArrowLeft, ArrowRight, Check, ChevronLeft, ChevronRight, CircleAlert, LockKeyhole, RefreshCw, Search, Shuffle, Ticket, X } from 'lucide-svelte';
  ...
  $: remaining = Math.max(0, allowance - selected.length);
  ...
  onMount(() => {
    ...
    if (draft?.raffleId === $page.params.id && Array.isArray(draft.selectedNumbers)) {
  ...
  async function pickForMe() {
    if (!remaining || suggesting) return;
    suggesting = true;
    try {
      const result = await api.post<{ numbers: number[] }>(`/raffles/${$page.params.id}/ticket-suggestions`, { count: Math.min(suggestionCount, remaining), exclude: selected });
      selected = [...new Set([...selected, ...result.numbers])].sort((a, b) => a - b);
      requestKey = '';
      saveDraft();
      notice = result.numbers.length ? 'Numbers added. You can change any of them before checkout.' : 'No numbers are available right now. Try refreshing.';
    } catch { notice = 'Could not choose numbers. Please try again.'; }
...
<div class="number-picker">
  ...
  <section class="picker-intro">
    <h1>A little luck.<br />Your own numbers.</h1>
    <p>Choose the numbers that mean something to you.</p>
  ...
  {#if resumePaymentId}
    <section class="resume-panel"><LockKeyhole size={20} /><div><h2>You have a checkout in progress</h2><p>Your numbers are held separately from this selection.</p><button on:click={resume}>Resume checkout <ArrowRight size={16} /></button></div></section>
  {/if}
  ...
  <section class="selection-tools" aria-label="Find ticket numbers">
    ...
    <div class="auto-pick"><div><Shuffle size={17} /><span>Let us choose</span></div><select aria-label="How many numbers to choose automatically" bind:value={suggestionCount} disabled={!remaining || purchasing}>{#each Array.from({ length: Math.max(1, remaining) }, (_, i) => i + 1) as n}<option value={n}>{n} ticket{n === 1 ? '' : 's'}</option>{/each}</select><button on:click={pickForMe} disabled={!remaining || purchasing || suggesting || !availability?.salesOpen}>{suggesting ? 'Picking…' : 'Pick for me'}</button></div>
  </section>
  ...
  <section class="numbers-section" aria-label="Available ticket numbers" aria-busy={refreshing}>
    <div class="grid-heading"><h2>Choose your numbers</h2><button class="icon-button" on:click={refresh} disabled={refreshing} aria-label="Refresh availability"><RefreshCw size={17} class={refreshing ? 'spin' : ''} /></button></div>
    <div class="legend"><span><i class="available-dot"></i>Available</span><span><i class="selected-dot"></i>Selected</span><span><i class="taken-dot"></i>Unavailable</span></div>
    {#if loading}
      <div class="number-grid" aria-label="Loading numbers">{#each Array(20) as _}<div class="number-skeleton"></div>{/each}</div>
    {:else if availability}
      <nav class="range-navigation" aria-label="Ticket number pages"><button class="icon-button" disabled={start <= 1 || refreshing} aria-label="Previous numbers" on:click={() => { start = Math.max(1, start - pageSize); refresh(); }}><ChevronLeft size={20} /></button><span>{numberLabel(availability.start)} – {numberLabel(availability.end)}</span><button class="icon-button" disabled={availability.end >= availability.ticketCap || refreshing} aria-label="Next numbers" on:click={() => { start += pageSize; refresh(); }}><ChevronRight size={20} /></button></nav>
      <div class="number-grid">
        {#each availability.numbers as row (row.number)}
          ...
          <button class="number-tile" class:selected={isSelected} class:unavailable={row.state !== 'available'} class:matched={search.trim() !== '' && Number(search) === row.number} class:conflict={conflicts.includes(row.number)}
            ...
            <span>{numberLabel(row.number)}</span>{#if isSelected}<Check size={12} strokeWidth={3} />{:else if row.state === 'owned'}<span class="tile-state">Yours</span>{:else if row.state !== 'available'}<span class="tile-state">{row.state.startsWith('held') ? 'Held' : 'Taken'}</span>{/if}
          </button>
        {/each}
      </div>
  ...
  <footer class="selection-footer">
    ...
    <p><LockKeyhole size={12} /> Reserved when you continue. Confirmed after payment.</p>
  </footer>
</div>
...
<style>
  ...
  .auto-pick { display: flex; align-items: center; gap: 8px; margin: 10px 0 24px; font-size: 12px; } .auto-pick > div { display: flex; align-items: center; gap: 8px; flex: 1; } .auto-pick select { min-height: 44px; background: transparent; color: var(--picker-ink); max-width: 85px; } .auto-pick button { min-height: 44px; color: #075d46; font-weight: 700; text-decoration: underline; text-underline-offset: 4px; }
  .grid-heading { display: flex; justify-content: space-between; align-items: center; } h2 { font-size: 16px; font-weight: 750; } .legend { display: flex; gap: 16px; flex-wrap: wrap; font-size: 11px; color: var(--picker-muted); } .legend span { display: flex; align-items: center; gap: 5px; } .legend i { width: 9px; height: 9px; border-radius: 3px; } .available-dot { background: white; border: 1px solid #778f84; } .selected-dot { background: #08765a; } .taken-dot { background: #c5cec9; }
  .range-navigation { display: flex; justify-content: space-between; align-items: center; margin-top: 10px; font-size: 12px; font-variant-numeric: tabular-nums; }
  .number-grid { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 8px; margin-top: 8px; }
  .number-tile, .number-skeleton { min-height: 56px; min-width: 48px; border-radius: 12px; } .number-tile { position: relative; display: flex; flex-direction: column; gap: 1px; justify-content: center; align-items: center; border: 1px solid #aebfb6; background: white; color: var(--picker-ink); font-size: 13px; font-weight: 650; font-variant-numeric: tabular-nums; transition: background-color 150ms ease-out, border-color 150ms ease-out, transform 150ms ease-out; } .number-tile.selected { background: #d4f4e6; border-color: #08765a; color: #064e3b; } .number-tile.unavailable:not(.selected) { background: #e8ede9; color: #59685f; border-color: transparent; } .tile-state { font-size: 9px; font-weight: 500; } .number-tile.matched { outline: 2px solid #193c33; outline-offset: 2px; } .number-tile.conflict { border-color: #a62940; background: #fff0f1; color: #8d2136; } .number-skeleton { background: #e6eee9; }
  ...
  button:disabled { cursor: default; } .continue-button:disabled { background: #e4ebe7; color: #627168; } .icon-button:disabled, .auto-pick button:disabled, select:disabled { opacity: .45; } button:not(:disabled):active { transform: scale(.97); } button:focus-visible, a:focus-visible, select:focus-visible, .number-search:focus-within { outline: 2px solid #08765a; outline-offset: 3px; } ::selection { background: #b9ead5; color: #193c33; } .spin { animation: spin 1s linear infinite; } @keyframes spin { to { transform: rotate(360deg); } }
  @media (max-width: 359px) { .number-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); } .auto-pick > div span { display: none; } .selection-footer { padding-inline: 16px; } }
  @media (prefers-reduced-motion: reduce) { .number-tile { transition: none; } .spin { animation: none; } button:not(:disabled):active { transform: none; } }
</style>
