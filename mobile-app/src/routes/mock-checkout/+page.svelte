...
<main class="checkout-shell min-h-[100dvh] bg-[#F2F5FA] text-[#080E49]">
  <div class="mx-auto flex min-h-[100dvh] w-full max-w-[480px] flex-col px-4 pb-[max(18px,env(safe-area-inset-bottom))] pt-[max(44px,var(--safe-top))]">
    <header class="flex h-11 items-center justify-between">
      <button type="button" class="pressable flex h-11 w-11 items-center justify-center rounded-full text-[#080E49]" aria-label={$_('mockCheckout.backAria')} on:click={() => { hapticLight(); history.back(); }}><ArrowLeft size={21} strokeWidth={2} /></button>
    ...
    {#if loadError}
      <section class="flex flex-1 flex-col items-center justify-center px-5 text-center" transition:fly={{ y: 10, duration: 220, easing: cubicOut }}>
        <span class="mb-4 flex h-14 w-14 items-center justify-center rounded-[20px] bg-[#ffe1e6] text-[#D8434C]"><CircleAlert size={25} /></span>
        ...
        <button type="button" class="pressable mt-6 h-12 w-full rounded-2xl bg-[#080E49] text-sm font-bold text-white" on:click={() => goto('/raffles')}>{$_('mockCheckout.browseRaffles')}</button>
      </section>
    {:else if outcome}
      <section class="flex flex-1 flex-col items-center justify-center px-5 text-center" transition:fly={{ y: 12, duration: 250, easing: cubicOut }}>
        <span class="checkout-result mb-5 flex h-20 w-20 items-center justify-center rounded-full {outcome === 'success' ? 'bg-[#DCE6FB] text-[#0135C6]' : 'bg-[#ffe1e6] text-[#D8434C]'}">
          {#if outcome === 'success'}<Check size={35} strokeWidth={2.4} />{:else}<CircleAlert size={32} />{/if}
        </span>
        <p class="text-[11px] font-bold uppercase tracking-[0.16em] text-[#0135C6]">{$_('mockCheckout.gatewayResponse')}</p>
        ...
        <div class="mt-7 flex items-center gap-2 text-xs font-semibold text-[#63716d]"><span class="checkout-pulse h-2 w-2 rounded-full bg-[#0129A3]"></span> {$_('mockCheckout.returningToApp')}</div>
      </section>
    {:else}
      <div class="flex flex-1 flex-col">
        <section class="pb-5 pt-4">
          <p class="text-[11px] font-bold uppercase tracking-[0.16em] text-[#0135C6]">{$_('mockCheckout.amountToPay')}</p>
        ...
        <section class="overflow-hidden rounded-[24px] border border-[#DCE4F5] bg-white shadow-[0_14px_34px_rgba(8,14,73,0.06)]">
          <div class="p-5">
            <div class="flex items-start gap-3.5">
              <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-[15px] bg-[#DCE6FB] text-[#0135C6]"><ReceiptText size={20} /></span>
          ...
          <dl class="space-y-3 px-5 py-4 text-[13px]">
            ...
            <div class="flex justify-between"><dt class="text-[#63716d]">{$_('mockCheckout.paymentMethod')}</dt><dd class="flex items-center gap-1.5 font-bold"><Smartphone size={14} class="text-[#0135C6]" /> {$_('mockCheckout.chapaTest')}</dd></div>
          ...
          <div class="flex items-center gap-2.5 border-t border-[#ECF1FA] bg-[#f8faf9] px-5 py-3 text-[11px] leading-4 text-[#63716d]"><ShieldCheck size={16} class="shrink-0 text-[#0135C6]" /> {$_('mockCheckout.issuedAfterConfirmation')}</div>
        </section>

        <div class="mt-auto pt-5">
          {#if submitError}<p class="mb-3 rounded-xl border border-[#f5c8cf] bg-[#fff0f2] px-3.5 py-3 text-xs leading-5 text-[#B23A45]" role="alert">{submitError}</p>{/if}
          <button type="button" disabled={submitting} on:click={() => finish('success')} class="pressable flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#080E49] text-sm font-extrabold text-white shadow-[0_12px_26px_rgba(8,14,73,0.16)] disabled:opacity-60">
...
<style>
  :global(body) { background: #F2F5FA; }
  .checkout-perforation::before, .checkout-perforation::after { background: #F2F5FA; }
