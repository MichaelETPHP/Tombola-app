<script lang="ts">
  ...
  import { language, languages, setLanguage } from '../stores/language.store.js';
...
<header class="flex items-center justify-between gap-2">
  ...
  <div class="flex shrink-0 items-center gap-0.5">
    ...
    <button
      ...
      aria-label={$_('header.tiktokAria')}
    >
      <TikTokIcon size={18} />
    ...
    <div class="relative">
      <button
        ...
        aria-label={$_('header.languageAria')}
      ...
      {#if langOpen}
        <button type="button" class="fixed inset-0 z-40 cursor-default" aria-label={$_('header.closeAria')} on:click={() => (langOpen = false)}></button>
        <div
    ...
    {:else}
      <a
        ...
        aria-label={$_('header.signInAria')}
      >
