...
export async function listUserTickets(userId: string): Promise<DbTicket[]> {
  const rows = await sql<(DbTicket & { categoryCode: string })[]>`
    SELECT t.*, r.category_code,
  ...
  `;
  // display_number is stored once at issuance (see payments.queries.ts's
  // completePaymentAndIssueTickets) — just read it, never recompute it.
  return rows.map((row) => ({ ...row, ticketCode: `${row.categoryCode}-${row.displayNumber}` }));
}
