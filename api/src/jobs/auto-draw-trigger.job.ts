...
async function startEligibleTierDraws(): Promise<void> {
  try {
    const eligible = await sql<{ raffleId: string; tier: number }[]>`
    ...
    for (const { raffleId, tier } of eligible) {
      try {
        ...
        logger.info(`Auto draw-trigger not ready for raffle ${raffleId} tier ${tier}: ${error instanceof Error ? error.message : error}`);
      }
    }
  } catch (error) {
    logger.error('Auto draw-trigger check failed', error);
  }
}
