...
function checkConfig(): CheckResult {
  ...
  if (env.NODE_ENV === 'production') {
    ...
    if (!env.MOCK_PAYMENTS) {
      if (!env.CHAPA_SECRET_KEY || env.CHAPA_SECRET_KEY.startsWith('your-'))
        issues.push('Real payments are enabled but CHAPA_SECRET_KEY is not configured');
      if (!env.CHAPA_WEBHOOK_SECRET || env.CHAPA_WEBHOOK_SECRET.startsWith('your-'))
        issues.push('Real payments are enabled but CHAPA_WEBHOOK_SECRET is not configured');
      if (env.CHAPA_MODE !== 'live')
        issues.push('Production real payments require CHAPA_MODE=live');
    }
  }
